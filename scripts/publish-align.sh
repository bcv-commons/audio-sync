...
# ── Pass 2: run manifests -> align/_runs/ (RE-ENABLED 2026-09-14) ──
#
# Contract B calls for publishing per-batch run manifests (scores,
# coverage) so a consumer doesn't have to rescan our raw timing/words
# output to know what happened. Disabled 2026-08-14 for lack of a
# confirmed consumer — bibles' pull_align_cache.py was built against
# exactly this contract (manifest-gated pulls) and flagged 2026-09-14
# that publishing had stopped after our two early 2026-07-15 test
# batches, which is what prompted checking back in on this. That's the
# confirmed consumer the original disable was waiting on, so back on it
# goes. Publishing the backlog now too — 3,548 real manifests had
# accumulated locally since the disable, 879 with actual per-chapter
# results (135,988 records total), none of it ever published.
#
# The shape-filtering logic below (only files matching {batch_id,
# results} — not shard-*/ or logs/, which also live in _runs/) already
# solves the "_runs/ is full of our own working data, not just
# manifests" problem, confirmed real 2026-08-14, so no changes needed
# beyond uncommenting.
